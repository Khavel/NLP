...
my_first_pat = r'([a-zA-Z0-9_.-]+)[\s]*(?:@|WHERE)[\s]*((?:cs[.|dot])*[a-zA-Z-]*.*)[\s]*(?:.|DOM)[\s]*-*[eE]-*[dD]-*[uU]'
my_second_pat = r'([0-9]{3})[\sa-zA-z-_.()]+([0-9]{3})[\sa-zA-z-_.()]+([0-9]{4})'
...
def process_file(name, f):
    ...
    for line in f:
        ...
        matches2 = re.findall(my_second_pat,line)
        for m in matches:
            group1 = m[0]
            group2 = m[1]
            if(group2.endswith('.')): #Fixes a problem with dlwh
                group2 = group2[:-1]
            email = '%s@%s.edu' % (group1,group2)
            email = email.replace('-', '')
